...
export default async function LabelSheetPage({ params }: { params: { printId: string } }) {
  const print = await prisma.print.findUnique({
    ...
    include: { editions: { orderBy: { number: "asc" } } },
  });
  if (!print) notFound();
      ...
      {labels.length === 0 ? (
        <p className="text-sm text-stone">
          {print.editionSize === null
            ? "This is an open edition — not limited or numbered, so there are no individual copies to label."
            : "No editions to label yet."}
        </p>
      ) : (
          ...
          {labels.map((label) => (
            <div key={label.number} className="border border-black p-3 flex items-center gap-3 break-inside-avoid">
              ...
              <div>
                ...
                <p className="text-xs text-stone">{print.drawerLocation || "Unfiled"}</p>
              </div>
            </div>
          ))}
        </div>

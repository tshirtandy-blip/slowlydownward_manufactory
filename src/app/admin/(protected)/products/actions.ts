...
/** Deletes a product and its Edition rows. Refuses if any Edition has ever
 * been attached to a real OrderItem — that means the product has genuine
 * order history, which this action must never silently destroy. (Products
 * with no orders yet, i.e. test/placeholder listings, delete cleanly.) */
export async function deletePrint(printId: string) {
  await requireStockAccess();
  ...
  if (print.orderItems.length > 0) {
    throw new Error("This product has order history and can't be deleted.");
  }

  await prisma.$transaction([
    prisma.edition.deleteMany({ where: { printId } }),
    prisma.print.delete({ where: { id: printId } }),
  ]);

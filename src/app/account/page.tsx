...
import { getCustomerCollection } from "@/lib/customer-collection";
import { AccountNav } from "@/components/account/AccountNav";
import { CollectionCarousel } from "@/components/account/CollectionCarousel";
...
export default async function AccountPage() {
  ...
  const collection = await getCustomerCollection((customer as any).id);
    ...
    <>
      ...
      <section className="mx-auto max-w-5xl px-6 py-16">
        <div className="flex flex-wrap items-baseline justify-between gap-4 mb-2">
          <h1 className="font-display text-3xl">Your collection</h1>
          <SignOutButton />
        </div>
        <p className="text-stone mb-8">Signed in as {customer.email}</p>
        <AccountNav active="/account" />
        <CollectionCarousel pieces={collection} />
      </section>
      <SiteFooter {...footer} />
    </>
  );
